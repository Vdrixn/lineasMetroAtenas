...
public class lineasMetroAtenas {
	...
	public static Estacion[] listaEstaciones() {
		...
		File f = new File("Estaciones.txt"); // ESTE FICHERO TENDRIA INFORMACION ESTILO "Estacion1;2 /n Estacion2;1 /n
												// Estacion3;0"
		try { // SI TIENE 0 EN LINEA ES QUE TIENE MAS DE UNA LINEA
			...
			while (sc.hasNextLine()) {
				...
				res[i] = new Estacion(frase[0], Integer.parseInt(frase[1]), 0, Integer.parseInt(frase[2]),
						Integer.parseInt(frase[1]));
			}
			sc.close();
